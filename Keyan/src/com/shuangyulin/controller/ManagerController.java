...
import com.shuangyulin.service.ManagerService;
import com.shuangyulin.po.Manager;
...
public class ManagerController extends BaseController {
	...
	/*ajax方式按照查询条件分页查询项目管理员信息*/
	@RequestMapping(value = { "/list" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void list(String managerUserName,String name,String telephone,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (managerUserName == null) managerUserName = "";
		if (name == null) name = "";
		if (telephone == null) telephone = "";
		if(rows != 0)managerService.setRows(rows);
		List<Manager> managerList = managerService.queryManager(managerUserName, name, telephone, page);
	    /*计算总的页数和总的记录数*/
	    managerService.queryTotalPageAndRecordNumber(managerUserName, name, telephone);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String managerUserName,String name,String telephone,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (managerUserName == null) managerUserName = "";
		if (name == null) name = "";
		if (telephone == null) telephone = "";
		List<Manager> managerList = managerService.queryManager(managerUserName, name, telephone, currentPage);
	    /*计算总的页数和总的记录数*/
	    managerService.queryTotalPageAndRecordNumber(managerUserName, name, telephone);
	    /*获取到总的页码数目*/
	    int totalPage = managerService.getTotalPage();
	...
	public String delete(@PathVariable String managerUserName,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
			  managerService.deleteManager(managerUserName);
	            request.setAttribute("message", "项目管理员删除成功!");
	            return "message";
	        } catch (Exception e) { 
	            e.printStackTrace();
	            request.setAttribute("error", "项目管理员删除失败!");
				return "error";
	...
	/*按照查询条件导出项目管理员信息到Excel*/
	@RequestMapping(value = { "/OutToExcel" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void OutToExcel(String managerUserName,String name,String telephone, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(managerUserName == null) managerUserName = "";
        if(name == null) name = "";
        if(telephone == null) telephone = "";
        List<Manager> managerList = managerService.queryManager(managerUserName,name,telephone);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Manager信息记录"; 
        String[] headers = { "用户名","姓名","性别","联系电话"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<managerList.size();i++) {
        	Manager manager = managerList.get(i); 
        	dataset.add(new String[]{manager.getManagerUserName(),manager.getName(),manager.getSex(),manager.getTelephone()});
        }
        /*
